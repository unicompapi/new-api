...
func (a *Adaptor) ConvertImageRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.ImageRequest) (any, error) {
	...
	case constant.RelayModeImagesGenerations:
		// When sequential image generation is requested, n=1 (injected by the validator)
		// would override sequential_image_generation_options.max_images and cause Seedream
		// to return only one image. Remove n so Seedream uses max_images instead.
...
// volcengineImageResp is used to extract billing data from image generation responses.
type volcengineImageResp struct {
...
// volcengineImageHandler reads the image generation response, extracts the actual
// image count for billing, and passes the body unchanged to the client.
func volcengineImageHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (any, *types.NewAPIError) {
	...
	fmt.Printf("[Seedream DEBUG] <<< Response Status: %d\n", resp.StatusCode)
	fmt.Printf("[Seedream DEBUG] <<< Response Body: %s\n", responseBody)

	var imgResp volcengineImageResp
	if unmarshalErr := common.Unmarshal(responseBody, &imgResp); unmarshalErr == nil {
		imageCount := imgResp.Usage.GeneratedImages
		if imageCount <= 0 {
			imageCount = len(imgResp.Data)
		}
		fmt.Printf("[Seedream DEBUG] <<< generated_images=%d, data_count=%d\n", imgResp.Usage.GeneratedImages, len(imgResp.Data))
		if imageCount > 0 {
