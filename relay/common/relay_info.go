...
func (t *TaskSubmitReq) HasImage() bool {
	...
	if t.Metadata != nil {
		if contentRaw, ok := t.Metadata["content"]; ok {
			if contentSlice, ok := contentRaw.([]interface{}); ok {
				for _, entry := range contentSlice {
...
// NormalizeTaskRequest fills prompt/content/metadata for Seedance-style payloads.
func (t *TaskSubmitReq) NormalizeTaskRequest() {
	t.normalizePromptFromDashScopeInput()
	t.normalizePromptFromContent()
	t.mergeContentIntoMetadata()
	t.mergeDashScopeParametersFromMetadata()
	t.mergeDurationIntoMetadata()
	t.mergeResolutionIntoMetadata()
}

// DashScopeParameterString reads a string field from metadata.parameters.
func (t *TaskSubmitReq) DashScopeParameterString(key string) string {
	return dashScopeParameterString(t.Metadata, key)
}

func dashScopeParameterString(metadata map[string]interface{}, key string) string {
	if metadata == nil {
		return ""
	}
	paramsRaw, ok := metadata["parameters"]
	if !ok {
		return ""
	}
...
func dashScopeParameterInt(metadata map[string]interface{}, key string) int {
	if metadata == nil {
		return 0
	}
	paramsRaw, ok := metadata["parameters"]
	...
	return dashScopeIntParam(params[key])
}

// mergeDashScopeParametersFromMetadata promotes metadata.parameters fields used by Ali/happyhorse billing.
func (t *TaskSubmitReq) mergeDashScopeParametersFromMetadata() {
	if t.Metadata == nil {
		return
	}
	if strings.TrimSpace(t.Resolution) == "" {
		if resolution := dashScopeParameterString(t.Metadata, "resolution"); resolution != "" {
			t.Resolution = resolution
		}
	}
	if t.Duration == 0 {
		if duration := dashScopeParameterInt(t.Metadata, "duration"); duration > 0 {
			t.Duration = duration
		}
	}
	if _, exists := t.Metadata["ratio"]; !exists {
		if ratio := dashScopeParameterString(t.Metadata, "ratio"); ratio != "" {
			t.Metadata["ratio"] = ratio
		}
	}
	for _, key := range []string{"size", "watermark", "prompt_extend", "seed", "audio"} {
		if _, exists := t.Metadata[key]; exists {
			continue
		}
		paramsRaw, ok := t.Metadata["parameters"].(map[string]interface{})
		if !ok {
			continue
		}
		if val, ok := paramsRaw[key]; ok && val != nil {
			t.Metadata[key] = val
		}
	}
}

func (t *TaskSubmitReq) normalizePromptFromDashScopeInput() {
	if t.Metadata == nil {
		return
	}
	inputRaw, ok := t.Metadata["input"]
	if !ok {
		return
	}
	...
	t.mergeDashScopeInputImages(input)
}

func (t *TaskSubmitReq) mergeDashScopeInputImages(input map[string]interface{}) {
	if len(t.Images) > 0 || strings.TrimSpace(t.Image) != "" {
		return
	}
	for _, key := range []string{"img_url", "first_frame_url"} {
		if url, ok := input[key].(string); ok && strings.TrimSpace(url) != "" {
			t.Images = append(t.Images, strings.TrimSpace(url))
		}
	}
	if mediaRaw, ok := input["media"].([]interface{}); ok {
		for _, item := range mediaRaw {
			...
			if url, ok := mediaMap["url"].(string); ok && strings.TrimSpace(url) != "" {
				t.Images = append(t.Images, strings.TrimSpace(url))
			}
		}
	}
...
func (t *TaskSubmitReq) normalizeDashScopeNativeFormat(data []byte) {
	if len(data) == 0 {
		return
	}
	...
	input, hasInput := raw["input"].(map[string]interface{})
	if hasInput {
		if strings.TrimSpace(t.Prompt) == "" {
			if prompt, ok := input["prompt"].(string); ok {
				t.Prompt = strings.TrimSpace(prompt)
		...
		if _, exists := t.Metadata["input"]; !exists {
			t.Metadata["input"] = input
		}
	}

	params, hasParams := raw["parameters"].(map[string]interface{})
	if !hasParams {
		return
	}
	if t.Metadata == nil {
	...
	if _, exists := t.Metadata["parameters"]; !exists {
		t.Metadata["parameters"] = params
	}
	t.mergeDashScopeParametersFromMetadata()
}

func dashScopeIntParam(v interface{}) int {
	switch n := v.(type) {
	case float64:
		return int(n)
	case int:
...
func (t *TaskSubmitReq) mergeVolcengineTopLevelParams(data []byte) {
	if len(data) == 0 {
		return
	}
	var raw map[string]interface{}
	...
	for _, key := range volcengineTopLevelTaskParamKeys {
		if _, exists := t.Metadata[key]; exists {
			continue
		}
...
func (t *TaskSubmitReq) mergeDurationIntoMetadata() {
	if t.Duration <= 0 {
		return
	}
	if t.Metadata == nil {
...
func (t *TaskSubmitReq) mergeResolutionIntoMetadata() {
	if strings.TrimSpace(t.Resolution) == "" {
		return
	}
	if t.Metadata == nil {
	...
	if _, exists := t.Metadata["resolution"]; !exists {
		t.Metadata["resolution"] = strings.TrimSpace(t.Resolution)
	}
}

func (t *TaskSubmitReq) normalizePromptFromContent() {
	if strings.TrimSpace(t.Prompt) != "" {
		return
	}
	for _, item := range t.Content {
		if item.Type == "text" && strings.TrimSpace(item.Text) != "" {
			t.Prompt = strings.TrimSpace(item.Text)
			return
		}
	}
	if t.Metadata == nil {
...
func (t *TaskSubmitReq) mergeContentIntoMetadata() {
	if len(t.Content) == 0 {
		return
	}
	if t.Metadata == nil {
	...
	contentBytes, err := common.Marshal(t.Content)
	if err != nil {
		return
	}
	...
	if len(aux.Metadata) > 0 {
		...
		if err := common.Unmarshal(aux.Metadata, &metadataStr); err == nil && metadataStr != "" {
			...
			if err := common.Unmarshal([]byte(metadataStr), &metadataObj); err == nil {
				...
				t.normalizeDashScopeNativeFormat(data)
				t.mergeVolcengineTopLevelParams(data)
				t.NormalizeTaskRequest()
				return nil
			}
		}
